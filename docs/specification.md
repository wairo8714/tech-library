...
1. 社員が一覧表示を実行する
2. アプリは登録されている本を取得する
3. アプリは件数、書籍ID、タイトル、作者、ジャンル、場所を表示する
...
3. アプリは一致した本の件数、書籍ID、タイトル、作者、ジャンル、場所を表示する
...
| 保管場所の番号 | 登録済みの保管場所一覧から選択する番号 |
| 新しい保管場所名 | 「新しい保管場所を追加する」を選んだ場合のみ入力する |
...
`find` コマンド実行後、以下の確認文を表示して対話形式で入力する。

```text
検索キーワードを入力してください(複数条件はカンマ区切り)
```
...
- 複数条件はカンマ区切りで受け取る
- 複数条件が指定された場合は AND 検索とする
- 検索結果は件数と各本の詳細を表示する
- 該当する本がない場合は `該当する本はありません` と表示する
...
- 登録されている本を一覧表示する
- 表示項目は検索結果と同じ形式にする
- 登録されている本がない場合は `登録されている本はありません` と表示する
...
- 表示形式は `list` の本情報に貸出No.や貸出状況を加えたものにする
